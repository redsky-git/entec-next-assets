				...
				<div className="mx-auto flex w-full  min-w-0 flex-1 flex-col gap-8 px-4 py-6 text-neutral-800 md:px-0 lg:py-8 dark:text-neutral-300">
					...
					<div className="w-full flex-1 *:data-[slot=alert]:first:mt-0">
						...
						<div className="flex flex-col gap-2 pt-6">
							...
							<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
								Client Component에서 useApi 훅을 사용하여 Route Handler를 호출하고, Route Handler 내부에는{' '}
								<strong>serverApi</strong>를 통해서 API를 호출할 수 있습니다.
							</p>
							<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
								다음 그림의 데이터 흐름을 봤을 때 Client Component에서는 <strong>useApi</strong> 훅을 사용하지만{' '}
								<strong>Route Handler</strong>에서는 <strong>serverApi</strong> 함수를 사용합니다.
							</p>
						...
						<div className="flex flex-col gap-2 pt-6">
							<div className="flex items-start justify-between">
								<h2
									...
									className="scroll-m-20 text-3xl font-semibold tracking-tight sm:text-3xl xl:text-3xl"
								>
									Server Component에서 serverApi 호출하기
								</h2>
								<div className="docs-nav bg-background/80 border-border/50 fixed inset-x-0 bottom-0 isolate z-50 flex items-center gap-2 border-t px-6 py-4 backdrop-blur-sm sm:static sm:z-0 sm:border-t-0 sm:bg-transparent sm:px-0 sm:pt-1.5 sm:backdrop-blur-none">
									&nbsp;
								</div>
							...
							<div className="flex justify-start pt-4">
								<Button onClick={() => router.push('/example/api-guides/server-api-server-component-ex')}>
									Server Component(serverApi) 예제 확인하기
								</Button>
							</div>
						</div>
						{/* example 블럭요서 END */}
