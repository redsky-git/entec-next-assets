...
interface ICallApiClientExProps {
	initialPostsData?: IPost[]; // Server Component에서 전달받은 초기 데이터
	test?: string;
}

const CallApiClientEx: IComponent<ICallApiClientExProps> = ({ initialPostsData = [] }): JSX.Element => {
	const [id, setId] = useState<number>(1);
	// Server Component에서 받은 초기 데이터로 state 초기화
	const [serverPostsData, setServerPostsData] = useState<IPost[]>(initialPostsData);
	// posts 데이터 조회
	...
	const fetchPosts = async () => {
		const response = await serverApi<any>(`posts`);
		console.log('response data', response.data);
		setServerPostsData(response.data);
	};

	// useEffect hooks - 클라이언트에서만 추가 데이터 fetch (선택적)
	// 초기 데이터는 Server Component에서 이미 받았으므로, 필요시에만 사용
	useEffect(() => {
		// 클라이언트에서 추가로 데이터를 갱신하고 싶을 때만 사용
		// fetchPosts();
	}, []);
									...
									<Card>
										<CardContent className="flex items-center justify-center">
											<div className="grid w-full gap-2">
												...
												<Textarea
													value={`Response Data (api서버 도메인/posts) : ${JSON.stringify(serverPostsData || [])}`}
													className="h-60"
						...
						<Card className="mt-4">
							<CardContent className="flex items-center justify-center">
								<div className="grid w-full gap-2">
									...
									{Array.isArray(serverPostsData) && serverPostsData.length > 0 ? (
										<div className="rounded-lg border bg-background p-4 shadow-sm">
											<ul
												className="divide-y divide-border overflow-y-auto"
												style={{ maxHeight: 340 }}
											>
												{serverPostsData.map((post: IPost) => (
													<li
														key={post.id}
